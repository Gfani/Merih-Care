/**
 * Real-Time Leaflet Tracking Map Engine
 * Replaces synthetic SVG mathematical approximations with real GPS OpenStreetMap tile tracking.
 */
...
// Default fallback coordinates (Addis Ababa, Ethiopia)
...
export function MockMap({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Add compact zoom control in top-left
      L.control
        .zoom({
          position: "topleft",
        })
  ...
  useEffect(() => {
    ...
    const patientCoord: [number, number] = patientLocation
      ? [patientLocation.lat, patientLocation.lng]
      : DEFAULT_CENTER;
    ...
    const showProvider = Boolean(providerLocation || (isTracking && patientLocation));

    if (showProvider) {
      const provCoord: [number, number] = providerLocation
        ? [providerLocation.lat, providerLocation.lng]
        : isArrived
        ? [patientCoord[0] + 0.0003, patientCoord[1] + 0.0003]
        : [patientCoord[0] - 0.008, patientCoord[1] - 0.008];
      ...
      // 3. Route Polyline
      if (showRoute && isTracking && !isArrived) {
      ...
      // 4. Fit bounds to contain both points
      try {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-3 left-3 z-10 text-[11px] font-medium text-slate-600 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-md shadow-sm border border-slate-200">
        📍 {patientLocation?.label || "Addis Ababa, Ethiopia"}
      </div>
    </div>
  );
}
